...
inat_case_study_plots <- function(
    species_vec,
    region_name,
  ...
  for (i in seq_along(names(per_species))) {
    ...
    title_italic_n <- function(sp, n) bquote(italic(.(sp)) ~ "(n =" ~ .(n) ~ ")")
    n_obs_sp   <- if (!is.null(res$n_obs)) res$n_obs else 0L
    title_expr <- title_italic_n(sp, n_obs_sp)
  ...
  combined_wrap <- wrap_plots(panels, ncol = species_cols) +
    plot_annotation(title = paste0(region_name, " - iNaturalist")) &
    theme(
  ...
  final_plot <- (
    (wrap_plots(panels, ncol = species_cols) / legend_seed) +
      ...
      plot_annotation(
        title = paste0(region_name, " — iNaturalist"),
...
p_hawaii <- inat_case_study_plots(
  species_vec   = hawaii_species_vec,
  region_name   = "Hawaii",
  region_level  = "state",
  country       = "United States of America",  # helps disambiguate
...
p_bahamas <- inat_case_study_plots(
