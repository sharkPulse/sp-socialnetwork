...
fit_shark_trend2 <- function(
  ...
  req <- c("month","year_observed","shark_observations", "validated_obs", "auto_obs", "total_observations","total_users")
  ...
  p <- ggplot(pred_ts_plot, aes(x = year_observed, y = spue_hat)) +
    geom_ribbon(aes(ymin = lo_plot, ymax = hi_plot), fill = "#FF0084", alpha = 0.15) +
    geom_line(linewidth = 1, color = "#FF0084") +
    ...
    labs(x = "Year", y = paste0("Sightings per 1,000 Posts")) +
    scale_x_continuous(
...
# 
# df <- m1$pred_ts %>% arrange(year_observed)
# 
# delta_spue <- df$spue_hat[nrow(df)] - df$spue_hat[1]
# delta_spue
# 
# print(m1$pred_ts, n = Inf)
